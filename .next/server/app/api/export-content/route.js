(()=>{var e={};e.id=444,e.ids=[444],e.modules={846:e=>{"use strict";e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},4870:e=>{"use strict";e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},3295:e=>{"use strict";e.exports=require("next/dist/server/app-render/after-task-async-storage.external.js")},9294:e=>{"use strict";e.exports=require("next/dist/server/app-render/work-async-storage.external.js")},3033:e=>{"use strict";e.exports=require("next/dist/server/app-render/work-unit-async-storage.external.js")},3639:(e,t,o)=>{"use strict";o.r(t),o.d(t,{patchFetch:()=>u,routeModule:()=>p,serverHooks:()=>h,workAsyncStorage:()=>g,workUnitAsyncStorage:()=>d});var a={};o.r(a),o.d(a,{POST:()=>l});var r=o(2706),n=o(8203),i=o(5994),s=o(9187);async function l(e){try{let{format:t,content:o}=await e.json(),a="",r="",n="";switch(t){case"html":a=c(o),r="text/html",n="html";break;case"markdown":a=function(e){var t,o,a;let r=e.type.charAt(0).toUpperCase()+e.type.slice(1),n=[`> **${r}**`,e.author?`> **Author:** ${e.author.name}${e.author.role?` (${e.author.role})`:""}`:"",`> **Published:** ${(t=e.createdAt||new Date().toISOString())?new Date(t).toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"}):""}`,e.categories?.length?`> **Categories:** ${e.categories.map(e=>`\`${e}\``).join(", ")}`:""].filter(Boolean).join("\n"),i=(o=e.content,a=e.title,o.replace(RegExp(`^#\\s*${a}\\s*
...
</details>`}(o),r="text/markdown",n="md";break;case"pdf":a=c(o),r="text/html",n="pdf";break;default:throw Error("Unsupported export format")}let i=new Blob([a],{type:r});return new s.NextResponse(i,{headers:{"Content-Type":r,"Content-Disposition":`attachment; filename="${o.title}.${n}"`}})}catch(e){return console.error("Error exporting content:",e),s.NextResponse.json({error:"Failed to export content"},{status:500})}}function c(e){var t;return e.type.charAt(0).toUpperCase(),e.type.slice(1),`<!DOCTYPE html>
...
`),"").trim(),a=e=>{let t=e.split("\n"),o=!1,a="",r=[],n=[];for(let e of t){let t=e.match(/^\s*[-*+]\s+(.*)/),i=e.match(/^\d+\.\s+(.*)/);if(t||i){o||(o=!0,a=t?"ul":"ol",r=[]);let e=t?t[1]:i[1];r.push(`<li>${e}</li>`)}else o&&(n.push(`<${a}>${r.join("\n")}</${a}>`),o=!1,r=[]),n.push(e)}return o&&n.push(`<${a}>${r.join("\n")}</${a}>`),n.join("\n")};return o.replace(/^[-]{3,}\s*$/gm,"").replace(/^(#+\s+.*?)\s*[-]{3,}\s*$/gm,"$1").replace(/^(.*?)\s*[-]{3,}\s*[-]{3,}\s*$/gm,"$1").replace(/^### (.*$)/m,"<h3>$1</h3>").replace(/^## (.*$)/m,"<h2>$1</h2>").replace(/^# (.*$)/m,"<h1>$1</h1>").replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/\*(.*?)\*/g,"<em>$1</em>").replace(/\[([^\]]+)\]\(([^)]+)\)/g,'<a href="$2" class="govuk-link">$1</a>').replace(/```([\s\S]*?)```/g,(e,t)=>`<pre><code>${t.trim()}</code></pre>`).replace(/`([^`]+)`/g,"<code>$1</code>").replace(/^>\s(.*)$/m,"<blockquote>$1</blockquote>").split("\n\n").map(e=>e.match(/^[-*+]\s/)||e.match(/^\d+\.\s/)?a(e):e).map(e=>{let t=e.trim();return t&&!t.startsWith("<")?`<p>${t}</p>`:t}).filter(e=>e.length>0).join("\n\n")})(e.content,e.title)}
      </div>
    </main>
  </div>
...
</html>`}let p=new r.AppRouteRouteModule({definition:{kind:n.RouteKind.APP_ROUTE,page:"/api/export-content/route",pathname:"/api/export-content",filename:"route",bundlePath:"app/api/export-content/route"},resolvedPagePath:"C:\\Users\\Lokesh-PC\\Downloads\\ADO PL\\app\\api\\export-content\\route.ts",nextConfigOutput:"standalone",userland:a}),{workAsyncStorage:g,workUnitAsyncStorage:d,serverHooks:h}=p;function u(){return(0,i.patchFetch)({workAsyncStorage:g,workUnitAsyncStorage:d})}},6487:()=>{},8335:()=>{}};var t=require("../../../webpack-runtime.js");t.C(e);var o=e=>t(t.s=e),a=t.X(0,[994,452],()=>o(3639));module.exports=a})();